...
class ConditionalRandomFieldNeural(ConditionalRandomFieldBackprop):
    ...
    def __init__(self, tagset, vocab, lexicon, rnn_dim, unigram=False):
        if unigram: raise NotImplementedError("BiRNN-CRF requires bigram features")
    ...
    def init_params(self) -> None:
        ...
        # Initialization: use gain < 1 to keep initial potentials reasonable
    ...
    def updateAB(self) -> None:
        # Pass: A and B are dynamic, calculated per position
        pass 
    ...
    def setup_sentence(self, isent) -> None:
        ...
        mp_w    = self.M_prime[:, 1:self.e+1]
        mp_hp   = self.M_prime[:, self.e+1:]
        
        for j in range(L - 1, -1, -1):
            act = mp_bias + (mp_w @ embs[j]) + (mp_hp @ h_bw[j+1])
            h_bw[j] = torch.sigmoid(act)
    ...
    def A_at(self, position: int, sentence) -> Tensor:
        ...
        # Safe-guard: cap exponent at e^20
    ...
    def B_at(self, position: int, sentence) -> Tensor:
        ...
        if i >= len(self.embedded_sentence): 
            # Boundary Safety
            return torch.full((self.k, self.num_words), 1e-20, device=dev)
        ...
        # Efficient return: Zeros (safe epsilon) everywhere, calc only for w_idx
        # Parent Hmm assumes we can access B[:, word]. 
